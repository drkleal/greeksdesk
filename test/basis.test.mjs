import test from 'node:test';import assert from 'node:assert/strict';import {calculateBasis,readBasis} from '../basis.mjs';
...
test('a nested chart thumbnail returns an actionable message without a basis',async()=>{
 const result=await readBasis('data:image/png;base64,aGVsbG8=',spx,'2026-09-04',{env:{OPENAI_API_KEY:'test'},request:async()=>({ok:true,json:async()=>({status:'completed',output:[{content:[{type:'output_text',text:JSON.stringify({...es,imageType:'chart_thumbnail'})}]}]})})});
 assert.equal(result.ok,false);assert.equal(result.basis,undefined);assert.match(result.message,/directly from DeepCharts/);
});
