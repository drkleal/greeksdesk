...
The analyst now returns additional price checkpoints independently of the six displayed map levels. These require the same exact price, identified price response, source and panel checks. Both map levels and checkpoints participate in the nearest-obstacle calculation, including obstacles within one point of the trigger. A nearer obstacle withholds the farther arrow and shows the actual distance to the first checkpoint. Its name opens the original evidence from the scenario card or the Level key; extra map lines are not added. Archived reads without checkpoint review cannot display an unchecked farther arrow. This improves consistency between structured evidence and the drawing; it does not prove that the analyst has identified every market obstacle.
