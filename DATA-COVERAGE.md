...
ES bars also include a deterministic New York date/time label alongside the original UTC timestamps. Session narratives use ET and must reconcile premarket, cash-session and after-close descriptions with the supplied timestamps and extrema. Summer/winter offsets and prior-evening date rollover are tested. This addresses an observed analysis error that described a 16:45 UTC midday reaction as a late-afternoon rebound.


## Earlier ES sessions

The same resolved raw contract supplies up to five earlier traded sessions from the preceding ten calendar days. The reader requests at most 256 hourly OHLCV rows, ending before the selected session starts at 18:00 New York on the prior date. It rejects a response at the row limit, mixed contracts, invalid prices, duplicated hours and overlapping session data. It does not stitch different contracts or fill absent hours/days. Hourly evidence is grouped into futures sessions and cached separately for 24 hours on the running server; changing the resolved contract requires new context.

The context panel shows the dated high, low and last hourly close for each returned session. Those summaries cover available trade bars; they are not cash-session extrema or official settlements. An hourly timestamp identifies the interval start, not the exact instant of its high/low. This follows [Databento’s OHLCV specification](https://databento.com/docs/schemas-and-data-formats/ohlcv).

A level selected from earlier API history must cite its session date, hourly bar timestamp and exact OHLC field. The server verifies that citation against the supplied same-contract bar. The level key and expanded explanation expose that origin. This proves the price was observed; it does not prove support/resistance or predict a return. The analyst must consider later crossings and price response before using an older reference as a conditional objective. Earlier history is omitted from prior-read comparisons to avoid duplicating the arrays or calling repeated history new market movement.

Live-app verification on September 6 returned five ESU6 sessions dated August 28 through September 3, each with 23 hourly trade bars. The combined historical estimate was $0.006857737899. The dated ranges appeared in the deployed desk and the completed analysis used them when assessing continuation beyond September 4's extremes. It withheld a farther downside objective because the nearby historical area had repeated crossings, rather than treating every prior price as untouched support.

## Inspecting a level's price evidence

Clicking an API-derived ES level opens its actual Databento trade bars with a yellow reference line. A white outline identifies the exact cited bar; selecting another bar shows that interval's OHLC prices and New York times. Session-summary references show their complete supplied interval without falsely identifying the opening bar as the time of a high or low. Missing trade intervals remain gaps. Original captured provider panels still open separately from their source buttons.

The latest full review combined three chart images and nine data sources and completed in 157 seconds. All 20 supplied panels/feeds were considered; one news panel was excluded with a stated reason. The UI reports exclusions separately from unreviewed sources. Concise output uses the same GPT-5.4 model and medium reasoning, with a 180-second server timeout and explicit failure diagnostics. This measured full-review latency is not suitable for claiming tick-by-tick analysis; data-only updates remain separate and neither refreshing price nor restoring a saved read makes an older plan current. Live-market latency and synchronization still require an observed-session check.
