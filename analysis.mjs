...
Every Databento-derived level/checkpoint must include apiOrigin {sourceId:'databento',sessionDate,timeframe,timestamp,field}. For an OHLC-derived price, copy the exact source bar-start timestamp and OHLC field holding that exact price. Earlier-session origins use timeframe '1h' and that earlier sessionDate. Current-session bar origins use '1m','5m','15m', or 'session'/'cash_session' with the summary's from timestamp. Trade-derived profile nodes and session VWAP use their specific profile timeframe, through timestamp and field described below; they do not require an OHLC match. One-hour highs/lows occurred somewhere within that hour, not necessarily at its starting minute. For a quote marker, provider model or chart-only annotation with no matching Databento origin, apiOrigin is null. A valid citation proves the stated observation, not that it is support/resistance: still explain the response pattern. Quote exact dates and ET intervals in derivation. Do not cite a similar price or a different timestamp just to fit the schema.
