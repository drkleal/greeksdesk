...
const instrumentEvidence=['contract_header','price_axis','underlying_selector','exposure_units_only','unknown'];
...
export const analysisSchema=obj({
 ...
 panels:arr(obj({id:str,sourceId:str,title:str,instrument:{type:'string',enum:instruments},instrumentEvidence:{type:'string',enum:instrumentEvidence},instrumentLabel:str,metricUnits:str,observedDate:str,dateRole:{type:'string',enum:dateRoles},status:{type:"string",enum:["usable","context","excluded"]},reason:str,shows:str,region:obj({x:{type:"number"},y:{type:"number"},width:{type:"number"},height:{type:"number"}})})),
...
export function validateAnalysis(value,packet){
 ...
 for(const p of panels){
  ...
  if(p.status==='usable'&&p.instrument==='ES'&&!['contract_header','price_axis'].includes(p.instrumentEvidence))throw Error('Native ES levels require an ES contract or price axis.');
 }
