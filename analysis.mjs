...
function nativeAPILevel(l,packet){const source=packet.sources.find(s=>l.sourceIds.includes(s.id)&&nativeESData(s));if(!source)return false;const d=source.data,prices=[d.latestPrice,...['high','low','open','close'].map(k=>d.session?.[k]),...['high','low','open','close'].map(k=>d.cashSession?.[k]),...d.recentBars.flatMap(b=>[b.open,b.high,b.low,b.close])];return prices.some(p=>Number.isFinite(p)&&Math.abs(p-l.price)<0.00001);}
