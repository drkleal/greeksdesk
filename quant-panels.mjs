...
export function tape(p,kind){
 ...
 const keys=kind==='equity'?['ticker','price','size','notionalValue','printType','tradeSide','isDelayedPrint','tradeTime']:kind==='oi'?['ticker','contractType','strikePrice','expirationDate','previousOpenInterest','currentOpenInterest','changeInOpenInterest','percentChangeInOpenInterest','sessionDate','createdTime']:['id','osi','ticker','contractType','strikePrice','expirationDate','premium','size','tradeSideCode','tradeConsolidationType','stockPrice','tradeTime','optionPrice','bidPrice','askPrice','exchange','tradeType','isCancelled','isComplex','isTied'];
 const normalized=p.data.map(r=>fields(r,keys)),excludedRows=[],rows=[];
 for(const r of normalized){
  const cancelled=kind==='options'&&(r.isCancelled===true||['CANCEL','CANCEL_LAST','CANCEL_OPEN','CANCEL_ONLY'].includes(r.tradeType));
  if(cancelled)excludedRows.push({...r,exclusionReason:'Provider marks this record cancelled or corrected; exclude it from trade conclusions and totals.'});else rows.push(r);
 }
 return {rowCount:rows.length,rows,...(kind==='options'?{tapeVersion:2,returnedRowCount:normalized.length,excludedRowCount:excludedRows.length,excludedRows}:{}),partial:!!p.nextSearchAfter,hasMore:!!p.nextSearchAfter,units:kind==='equity'?'Prices and notional in USD; size in shares':kind==='oi'?'Open interest in contracts; percentChangeInOpenInterest in percent, verified against count changes':'Premium in USD; size in contracts',limitation:'Bounded first page, ordered by the stated request. Not the complete tape. Quote-side classification does not establish buyer identity or opening/closing intent.'+(kind==='options'?' Cancelled/corrected records are retained separately as excluded evidence. Complex or tied trades cannot establish standalone directional intent. Missing condition flags remain unknown; false is not inferred. Consolidated and unconsolidated views overlap and must not be added together.':'')};
}
