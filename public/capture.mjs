...
export async function loadChartDrafts(date){const db=await database();try{return await new Promise((resolve,reject)=>{const r=db.transaction('drafts').objectStore('drafts').getAll();r.onsuccess=()=>{const drafts=r.result.filter(s=>['paste-dg','es-snapshot','paste-vp','paste-vp-prior','paste-vp-composite'].includes(s.id)&&s.sessionDate===date),legacy=r.result.find(s=>s.id==='es'&&s.sessionDate===date);if(legacy&&!drafts.some(s=>s.id==='es-snapshot'))drafts.push({...legacy,id:'es-snapshot'});resolve(drafts);};r.onerror=()=>reject(r.error);});}finally{db.close();}}
