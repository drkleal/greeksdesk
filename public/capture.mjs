...
export async function saveChartDraft(source,id=source?.id){const db=await database();try{await new Promise((resolve,reject)=>{const tx=db.transaction('drafts','readwrite');if(source)tx.objectStore('drafts').put(source);else tx.objectStore('drafts').delete(id);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);});}finally{db.close();}}
export async function loadChartDrafts(date){const db=await database();try{return await new Promise((resolve,reject)=>{const r=db.transaction('drafts').objectStore('drafts').getAll();r.onsuccess=()=>{const drafts=r.result.filter(s=>['paste-dg','es-snapshot','es-five-minute','paste-vp','paste-vp-prior','paste-vp-composite'].includes(s.id)&&(s.sessionDate===date||s.id==='paste-dg')),legacy=r.result.find(s=>s.id==='es'&&s.sessionDate===date);if(legacy&&!drafts.some(s=>s.id==='es-snapshot'))drafts.push({...legacy,id:'es-snapshot'});resolve(drafts);};r.onerror=()=>reject(r.error);});}finally{db.close();}}
...
export async function saveESDraft(source){const db=await database();try{await new Promise((resolve,reject)=>{const tx=db.transaction('drafts','readwrite'),store=tx.objectStore('drafts');if(source){store.put({...source,id:'es'});store.put({...source,id:'es-snapshot'});}else{store.delete('es');store.delete('es-snapshot');}tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);});}finally{db.close();}}
