...
export function renderMap(host,analysis,instrument,onSelect){
 ...
 for(const s of analysis.scenarios){const from=levels.find(l=>l.id===s.triggerId),to=levels.find(l=>l.id===s.targetId);if(!from||!to||from.price===to.price||s.status==='insufficient')continue;const x=s.direction==='up'?450:s.direction==='down'?590:690;svg.append(svgNode('path',{d:`M ${x} ${y(from.price)} L ${x+40} ${y((from.price+to.price)/2)} L ${x+65} ${y(to.price)}`,fill:'none',stroke:s.direction==='up'?'#22efb6':s.direction==='down'?'#ff5788':'#ffdc68','stroke-width':3,'stroke-dasharray':'6 4','marker-end':'url(#arrow-'+s.direction+')'}));}
